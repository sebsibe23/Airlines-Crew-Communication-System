...
router.get('/', async (req, res) => {
    const { empID, intstartDT } = req.body;

    if (!empID || !intstartDT) {
        return res.status(400).send('Missing required parameters: empID or intstartDT');
    }

    try {
        const roster = await GetCrewRoster(empID, intstartDT);
        res.send({ roster });
    } catch (error) {
...
module.exports.GetCrewRoster = GetCrewRoster;