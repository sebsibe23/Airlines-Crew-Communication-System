...
async function getCrewOnBoard(flightNumber, startDT, departureAirport) {
    ...
    try {
        ...
        const ns10 = 'http://stl.sabre.com/AirCrews/CrewManager10/CommonDataTypes/v10';
        const ns15 = 'http://stl.sabre.com/AirCrews/CrewManager10/CrewRosterService/v10';

        const crewOB = result['s:Envelope']['s:Body'][0]['GetCrewOnFlightRS'][0]['Crew'];
        let varFlightData = [];

        crewOB.forEach(crew => {
            const staffNumber = crew.$.staffNumber;
            const firstName = crew['CrewPersonalInfo'][0].$.firstName;
            const crewOnFlight = result['s:Envelope']['s:Body'][0]['GetCrewOnFlightRS'][0]['CrewOnFlight'].find(x => x.$.staffNumber === staffNumber);
...
// Example usage
getCrewOnBoard('500', '2024-06-16T00:00:00Z', 'ADD').then(console.log);
