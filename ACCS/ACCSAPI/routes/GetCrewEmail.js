...
router.get('/', async (req, res) => {
  ...
  try {
    const emailInfo = await getEmpEmail(STRempID);
    res.send(emailInfo);
  } catch (error) {
...
async function getEmpEmail(STRempID) {
...
//console.log(requestData);
  try {
    ...
    crewList.forEach((crewDataD) => {
      ...
      const crewEmail = crewDataD['ns11:CrewEmails'][0]['ns11:CrewEmail'] || [];
      const PrimarycrewEmail = crewEmail.find((pCrewEmail) => pCrewEmail.$.emailType === 'PRIMARY');
      if (PrimarycrewEmail) {
        SendMSG += `||Email: ${PrimarycrewEmail._}`;
      }
    });
...
module.exports.getEmpEmail = getEmpEmail;
